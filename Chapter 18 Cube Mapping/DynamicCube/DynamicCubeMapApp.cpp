...
private:
	...
	float mDistToCube{ 1.0f };
	RenderItem* mMirrorCube = nullptr;
};
...
bool DynamicCubeMapApp::Initialize()
{
	...
	BuildCubeFaceCamera(0.0f, 3.0f, 0.0f);
...
void DynamicCubeMapApp::Update(const GameTimer& gt)
{
	...
	//XMMATRIX cubeScale = XMMatrixScaling(0.2f * mDistToCube, 0.2f * mDistToCube, 0.2f * mDistToCube);
	//XMMATRIX cubeOffset = XMMatrixTranslation(0.0f, 3.0f, 0.0f);
	//XMMATRIX cubeRotation = XMMatrixRotationY(0.0f);
	//XMStoreFloat4x4(&mMirrorCube->World, cubeScale * cubeOffset * cubeRotation);
	//mMirrorCube->NumFramesDirty = gNumFrameResources;
...
void DynamicCubeMapApp::UpdateCubeMapFacePassCBs()
{
	for(int i = 0; i < 6; ++i)
	{
		...
		mCubeMapCamera[i].SetLens(0.5f * XM_PI * (mDistToCube / 10.0f), 1.0f, 0.1f, 1000.0f);
		mCubeMapCamera[i].UpdateViewMatrix();

		// XMMATRIX scale = XMMatrixScaling(mDistToCube, mDistToCube, mDistToCube);
...
void DynamicCubeMapApp::BuildRenderItems()
{
	...
	XMStoreFloat4x4(&globeRitem->World, XMMatrixScaling(1.8f, 1.8f, 1.8f)*XMMatrixTranslation(0.0f, 3.0f, 0.0f));
...
void DynamicCubeMapApp::UpdateCameraDistToCube()
{
	XMFLOAT3 cubePosition{ mMirrorCube->World._41, mMirrorCube->World._42, mMirrorCube->World._43 };
	XMFLOAT3 distVector{ 0.0f, 0.0f, 1.0f };
	// Length is stored in all components
	XMStoreFloat3(&distVector, XMVector3Length(XMLoadFloat3(&cubePosition) - XMLoadFloat3(&mCamera.GetPosition3f())));
	mDistToCube = distVector.x;
}